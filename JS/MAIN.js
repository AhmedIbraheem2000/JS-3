...
function dispaly(){
 ...
 for(var i=0;i<allproduct.length;i++){
    ...
    <th><button type="button" class="btn btn-outline-success" id="visit" onclick="visititem(${i})">
    <i class="fa-solid fa-eye"></i>
    Visit</button></th>
    <th><button type="button" class="btn btn-outline-danger" id="delete" onclick="deletitem(${i})">
...
function vaild(){
    var nameregex =/^[A-Z a-z][a-z A-Z]{1,9}$/gm;
...
function searchInfo(){
    var searchVal = searchletters.value
    var box = ""
    for(var i = 0 ; i< allproduct.length;i++ ){
        if( allproduct[i].name.includes(searchVal) == true){
            box +=`
            <tr>
            <th>${i +1}</th>
            <th>${allproduct[i].name.replace(searchVal,"<span class='bg-warning'>"+searchVal+"</span>")}</th>
            <th><button type="button" class="btn btn-outline-success" id="visit" onclick="visititem(${i})">
            <i class="fa-solid fa-eye"></i>
            Visit</button></th>
            <th><button type="button" class="btn btn-outline-danger" id="delete" onclick="deletitem(${i})">
